...
export const searchTool = createTool({
  id: "search-tool",
  description:
    "Tavily を使用して特定のクエリに関する Web 情報を検索し、要約されたコンテンツを返します",
    ...
    results: z
      .array(
        z.object({
          ...
          url: z.string().describe("検索結果の URL"),
  ...
  execute: async (inputData, context) => {
    ...
    logger?.info("Web 検索ツールを実行中");
    const { query } = inputData;
    try {
      if (!process.env.TAVILY_API_KEY) {
        logger?.error("エラー: 環境変数に TAVILY_API_KEY が見つかりません");
        return { results: [], error: "API キーが見つかりません" };
      }
      logger?.info(`Web を検索中: "${query}"`);
